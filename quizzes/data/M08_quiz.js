window.QUIZ = {
  ...
  questions: [
    ...
    },
    {
      ...
      type: "mc",
      typeLabel: "Coding",
      prompt: "This snippet mimics how a sequential CrewAI crew passes each task's output forward as accumulated context, like a relay baton. What does it print?",
      code: "tasks = [\"research\", \"analyze\", \"write\"]\ncontext = []\nfor t in tasks:\n    inherited = list(context)\n    context.append(t + \"_output\")\n    print(t, \"sees\", len(inherited))\nprint(\"final\", len(context))",
      options: [
        {
          letter: "A",
          text: "research sees 0 / analyze sees 1 / write sees 2 / final 3",
          explanation: "Correct: each task sees the outputs produced before it (0, then 1, then 2), and after all three run the accumulated context holds 3 outputs."
        },
        {
          letter: "B",
          text: "research sees 0 / analyze sees 0 / write sees 0 / final 3",
          explanation: "This assumes each task starts from empty context. But context accumulates: it is appended to inside the loop before the next iteration, so analyze sees 1 prior output and write sees 2."
        },
        {
          letter: "C",
          text: "research sees 1 / analyze sees 2 / write sees 3 / final 3",
          explanation: "This counts the current task's own output as already visible. The inherited list is copied before appending the current output, so each task sees only what ran before it: 0, 1, then 2."
        },
        {
          letter: "D",
          text: "research sees 0 / analyze sees 1 / write sees 2 / final 2",
          explanation: "The per-task counts are right, but the final count is wrong. Three tasks each append one output, so the accumulated context ends with 3 items, not 2."
        }
      ],
      correctIndex: 0
    }
